...
const causes = (rankings.causes || []).slice(0, 6);
const vehicles = (rankings.vehicles || []).slice(0, 10);
const timeGroups = (distributions.rankedTimeBuckets || []).slice(0, 7)
  .map(item => ({ ...item, label: `${item.label.replace("-", " - ")} 時` }));
const ageGroups = (distributions.ageGroups || []).slice(0, 5);
...
const periodShort = payload.period?.short || period;
const fatalRows = (payload.a1Details || []).slice(0, 7).map(item => [
  String(item.number), item.time, item.location, item.cause, item.vehicle, `${item.deaths || "1"}人死亡`,
]);
while (fatalRows.length < 7) fatalRows.push([String(fatalRows.length + 1), "", "", "", "", ""]);

const summary = payload.narrative?.summary || "本期無可用事故摘要。";
const enforcement = payload.narrative?.enforcement || "本期無可用勤務建議。";
...
setText(2, "TextBox 18", `較前期變化：${overview.changes?.total?.text || "未提供同期資料"}`);
...
setText(2, "TextBox 27", `整體狀況：${payload.narrative?.summaryShort || summary}`);
setText(2, "TextBox 29", `核心防制重點：${payload.narrative?.coreFocus || `${causes[0]?.label || "主要肇因"}及${topVehicle.label}為本期優先關注項目。`}`);
